...
if (-not (Get-Command emcc -ErrorAction SilentlyContinue)) {
  throw "Emscripten compiler 'emcc' hittades inte i PATH."
}

if (-not (Test-Path (Join-Path $vendorRoot 'src\pt2_main.c'))) {
  throw "Vendorerad p2-clone-kod saknas. Kör scripts\\sync-upstream.ps1 först."
}

New-Item -ItemType Directory -Force $outDir | Out-Null

$sources = @(
  (Get-ChildItem (Join-Path $vendorRoot 'src') -Filter *.c -File | ForEach-Object FullName)
  (Get-ChildItem (Join-Path $vendorRoot 'src\modloaders') -Filter *.c -File | ForEach-Object FullName)
  (Get-ChildItem (Join-Path $vendorRoot 'src\smploaders') -Filter *.c -File | ForEach-Object FullName)
  (Get-ChildItem (Join-Path $vendorRoot 'src\gfx') -Filter *.c -File | ForEach-Object FullName)
)

$sources = $sources | Where-Object { $_ -notmatch 'bmp2pth\.c$' }

$outputJs = Join-Path $outDir 'pt2clone.js'
$preloadConfig = "{0}@/protracker.ini" -f (Join-Path $vendorRoot 'release\other\protracker.ini')
...
if ($LASTEXITCODE -ne 0) {
  throw "emcc-byggningen misslyckades."
}
