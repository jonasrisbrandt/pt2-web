...
#ifdef __EMSCRIPTEN__
static bool injectedMouseStateActive = false;
static int32_t injectedMouseX, injectedMouseY;
static uint32_t injectedMouseButtons;
#endif

#ifdef __EMSCRIPTEN__
EM_JS(double, getCanvasCSSWidth, (void), {
	if (!Module['canvas'])
		return 320.0;

	const rect = Module['canvas'].getBoundingClientRect();
	return rect.width > 0 ? rect.width : (Module['canvas'].width || 320);
});

EM_JS(double, getCanvasCSSHeight, (void), {
	if (!Module['canvas'])
		return 255.0;

	const rect = Module['canvas'].getBoundingClientRect();
	return rect.height > 0 ? rect.height : (Module['canvas'].height || 255);
});
#endif
...
{
#ifdef __EMSCRIPTEN__
	const double cssW = getCanvasCSSWidth();
	const double cssH = getCanvasCSSHeight();

	if (cssW > 0.0) video.dMouseXMul = (double)SCREEN_W / cssW;
	if (cssH > 0.0) video.dMouseYMul = (double)SCREEN_H / cssH;
#else
...
void setMouseXYFromWindowCoords(int32_t mx, int32_t my)
{
	mouse.absX = mx;
	mouse.absY = my;
	mouse.rawX = mx;
...
void setInjectedMouseState(int32_t mx, int32_t my, uint32_t buttonState)
{
#ifdef __EMSCRIPTEN__
	injectedMouseStateActive = true;
	injectedMouseX = mx;
	injectedMouseY = my;
	injectedMouseButtons = buttonState;
#else
	(void)mx;
	(void)my;
	(void)buttonState;
...
void readMouseXY(void)
{
...
#ifdef __EMSCRIPTEN__
	if (injectedMouseStateActive)
	{
		mx = injectedMouseX;
		my = injectedMouseY;
		mouse.buttonState = injectedMouseButtons;
	...
	setMouseXYFromWindowCoords(mx, my);
}
