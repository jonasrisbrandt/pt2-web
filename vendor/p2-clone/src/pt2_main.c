...
static void handleInput(void)
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		{
#ifdef __EMSCRIPTEN__
			setMouseXYFromWindowCoords(event.button.x, event.button.y);
#endif
		...
		{
			setMouseXYFromWindowCoords(event.motion.x, event.motion.y);
		}
#endif
